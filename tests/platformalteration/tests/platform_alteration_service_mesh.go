...
var _ = Describe("platform-alteration-service-mesh-usage-installed", func() {
	...
	execute.BeforeAll(func() {
		if _, exists := os.LookupEnv("NON_LINUX_ENV"); !exists {
			By("Install istio")
			cmd := exec.Command("/bin/bash", "-c", "curl -L https://istio.io/downloadIstio | ISTIO_VERSION=1.18.0 sh - "+
				"&& istio-1.18.0/bin/istioctl install --set profile=demo -y")
			err := cmd.Run()
...
var _ = Describe("platform-alteration-service-mesh-usage-uninstalled", func() {
	...
	It("istio is not installed", func() {
		...
		if err == nil {
			...
			if _, exists := os.LookupEnv("NON_LINUX_ENV"); !exists {
				cmd := exec.Command("/bin/bash", "-c", "curl -L https://istio.io/downloadIstio | ISTIO_VERSION=1.18.0 sh - "+
					"&& istio-1.18.0/bin/istioctl uninstall -y --purge")
				err := cmd.Run()
